...
def plot_scatter_diagram(which_fig, x, y, x_label = 'x', y_label = 'y', title = 'title', style_list = None):
	...
	assert len(x) == len(y)
	if style_list is not None:  #modified from "！=" to "is not"
		assert len(x) == len(style_list) and len(styles) >= len(set(style_list))
	plt.figure(which_fig)
	plt.clf()
	if style_list is None:   #modified from "==" to "is"
		plt.plot(x, y, color=styles[0], linestyle='None', marker='.')   #modified from "linestyle='.' " to "linestyle='None'"
	else:
		...
		for idx, cls in enumerate(clses):
			...
			plt.plot(xs[cls], ys[cls], color=style, linestyle='None', marker='.')#modified from "linestyle='.' " to "linestyle='None'"
	plt.title(title)
	plt.xlabel(x_label)
	plt.ylabel(y_label)
	plt.show()
